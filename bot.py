...
# ===== НАСТРОЙКА КОМАНД =====
bot.set_my_commands([
    telebot.types.BotCommand("/start", "Начать игру"),
    telebot.types.BotCommand("/profile", "Мой профиль"),
    telebot.types.BotCommand("/attack", "Заработать монеты"),
    telebot.types.BotCommand("/business", "Магазин бизнесов"),
    telebot.types.BotCommand("/mybusiness", "Мои бизнесы"),
...
@bot.message_handler(func=lambda message: message.text.lower() in ["группировка"])
def group_rus(message):
    start(message)
...
@bot.message_handler(func=lambda message: message.text in ["🏷️ Лейбл"])
